...
⚠️ כלל קריטי – הפרדת שם עובד מגוף מנהל (Spatial Context Awareness):
- שם העובד (full_name): חלץ אך ורק מאזור הכותרת של התלוש, ליד התווית "שם העובד", "שם", "עובד/ת". גם אם השם הוא "הראל", "מנורה" או כל שם שנראה כמו חברת ביטוח – אם הוא מופיע ליד "שם העובד" זהו שם הלקוח.
- גוף מנהל (destination_institution): חלץ אך ורק מטבלאות הניכויים או ההפרשות ("ניכויי עובד", "הפרשות מעסיק"). אם "הראל" מופיע ליד אחוז (%) או סכום כספי (₪) – זהו גוף מנהל, לא שם עובד.
- אם שם העובד והגוף המנהל זהים (למשל: עובד בשם "הראל כהן" מבוטח ב"הראל ביטוח") – החזר את שניהם בנפרד כשדות עצמאיים ב-JSON. אל תדלג על אף אחד מהם.
